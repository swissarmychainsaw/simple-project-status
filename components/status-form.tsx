...
import {
  ...
  PROJECT_KEYS,
  normalizeBannerKey,
  getMergedProfileChanges, // ← used to pull defaults (title/summary) from profiles
  type BannerKey,
...
const emptyFormData: FormData = {
  ...
  // audio fields from your shared type (harmless defaults)
  audioMp3Url: "",
  audioValidatedUrl: "",
  audioPlayer: "",
};
...
export default function StatusForm({ onTitleChange }: Props) {
  ...
  // Prefill programTitle & programSummary when Project changes,
  // pulling values from projectProfiles via getMergedProfileChanges.
  useEffect(() => {
    const key = designOptions.optBannerId as BannerKey;
    if (!key) return;

    // The helper’s exact return shape can vary; read defensively.
    // @ts-expect-error: profiles helper is intentionally loose here.
    ...
    const nextSummary =
      from.programSummary ??
      from.summary ??
      formData.programSummary;

    setFormData((prev) => ({
      ...prev,
      programTitle: String(nextTitle ?? prev.programTitle ?? ""),
      programSummary: String(nextSummary ?? prev.programSummary ?? ""),
    }));
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                <label className="text-sm font-medium">Last Status</label>
                <select
                  className="mt-1 block w-full border rounded-md px-3 py-2 bg-white"
                  value={formData.lastStatus}
                  onChange={(e) => update("lastStatus", e.target.value as any)}
                >
                  <option>Green</option>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    update("currentStatus", e.target.value as any)
                  }
                >
                  <option>Green</option>
                  <option>Yellow</option>
